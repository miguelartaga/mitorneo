...
if (!tournamentId || !categoryArg) {
  console.error("Uso: node scripts/repair-playoff-bracket.js <tournamentId> <categoryId|abbrev>");
  process.exit(1);
}
...
const main = async () => {
  ...
  if (!slots.length) {
    console.error("No hay slots de playoff para esta categoria.");
    process.exit(1);
  }
  ...
  slots.forEach((slot) => {
    slotByPosition.set(slot.position, slot.entrantId ?? null);
  });
  ...
  const matchCount = Math.min(roundOneMatches.length, Math.floor(bracketSize / 2));

  for (let i = 0; i < matchCount; i += 1) {
    const match = roundOneMatches[i];
    const seedA = seedOrder[i * 2] ?? null;
    const seedB = seedOrder[i * 2 + 1] ?? null;
    const nextTeamAId = seedA ? slotByPosition.get(seedA) ?? null : null;
    const nextTeamBId = seedB ? slotByPosition.get(seedB) ?? null : null;
    const completed = isMatchComplete(match);
    if (completed && (match.teamAId !== nextTeamAId || match.teamBId !== nextTeamBId)) {
      warnings.push(`R1 match ${match.id} ya tiene resultado, no se actualiza.`);
      continue;
    }
    if (match.teamAId !== nextTeamAId || match.teamBId !== nextTeamBId) {
      match.teamAId = nextTeamAId;
      match.teamBId = nextTeamBId;
      updates.push({ id: match.id, teamAId: nextTeamAId, teamBId: nextTeamBId });
    }
  }
...
main()
  .catch((error) => {
    console.error("Error al reparar playoffs:", error);
    process.exitCode = 1;
  })
  .finally(async () => {
